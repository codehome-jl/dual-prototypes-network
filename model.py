...
class Mutildimension_attention(nn.Module):
    def __init__(self, args, temporal_set_size=3):
        ...
        self.W_Q = nn.Linear(self.args.trans_linear_in_dim * temporal_set_size, self.args.trans_linear_in_dim * temporal_set_size)
        self.W_K = nn.Linear(self.args.trans_linear_in_dim * temporal_set_size, self.args.trans_linear_in_dim * temporal_set_size)
        self.W_V = nn.Linear(self.args.trans_linear_in_dim * temporal_set_size, self.args.trans_linear_in_dim * temporal_set_size)
    ...
    def forward(self, support_set, support_labels, queries):
        ...
        for label_idx, c in enumerate(unique_labels):
            ...
            for ids,k_ch in enumerate(class_n_k):
                ...
                #v_ch = class_n_v[ids]
                class_sco = torch.matmul(q_set,k_ch.transpose(-2, -1))/math.sqrt(self.args.trans_linear_in_dim * self.temporal_set_size)
